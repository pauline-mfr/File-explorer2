...
$file_open = fopen($my_item, "r") or die("Unable to open file!"); // on l'ouvre avec fopen(chemin vers le fichier, mode de lecture)  __ Sinon affiche un message d'erreur
   echo fread($file_open,filesize($my_item)); // fread lit le fichier ouvert dans sa totalité (filesize de $my_file)
...
$my_item = $_GET['file_to_open'];
$file_scan = new SplFileInfo($my_item);
$file_ext = ($file_scan->getExtension());

if ($file_ext == 'txt'){ // SI TXT
  $file_open = fopen($my_item, 'r') or die ('Unable to open this file');
  echo fread($file_open, filesize($my_item));
  fclose($file_open);
} else { // SI IMG
  $img_cwd = explode(DIRECTORY_SEPARATOR, $my_item, 6);
  $img_src = $img_cwd[5];
